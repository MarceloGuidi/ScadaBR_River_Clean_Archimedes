...
//Bibliotecas adicionadas para conexão WiFi Manager e ModbusIP
...
// Declaração de variáveis de tipo inteiro
...
void setup()
{
	mb.config("River_Clean","123456789");//Informações da rede WiFi para comunicação modbus
	mb.addCoil(LED_COIL);//adicionando COIL
  ...
  WiFi.begin("River_Clean", "123456789");
  ...
  {
    delay(500);        // tempo de 1/2 segundo
    Serial.print("."); // Imprime no monitor serial o caractere "." durante as tentativas de conexão sem sucesso
  }
  Serial.println();                             // Imprime linha em branco
  Serial.print("conectado, Endereço de IP:  "); // imprime mensagem a "DESKTOP-F3R7F79 6901","87944>Cm"  no monitor serial
...
void loop()
{
  ...
  if (A == 0)
  {
    ...
    delay(100);              // Aguarda um tempo de 100 milisegundos de interrupção de execução do código
  }
